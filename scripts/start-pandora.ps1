...
if ($LASTEXITCODE -ne 0) {
    throw "Hidden VBS launcher failed with exit code $LASTEXITCODE."
}

$listenerPid = Wait-PandoraPortListenerPid -Port $Port -TimeoutSeconds 20
if (-not $listenerPid) {
    Write-Warning 'Hidden VBS launcher did not open the port. Falling back to direct hidden Java launch.'
    ...
    $listenerPid = Wait-PandoraPortListenerPid -Port $Port -TimeoutSeconds 40
}
if (-not $listenerPid) {
