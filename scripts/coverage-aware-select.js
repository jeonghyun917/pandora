...
const { selectCoveragePolicy } = require('./lib/coverage-aware-selection');
...
function main(argv = process.argv.slice(2)) {
  ...
  const selection = selectCoveragePolicy({
    manifestInfo,
  ...
  console.log(`[coverage-aware-select] ${selection.status} ${options.outputPath}`);
  return selection;
}
...
if (require.main === module) {
  try {
    ...
    console.error(`[coverage-aware-select] ERROR ${error?.message ?? error}`);
    process.exitCode = 1;
  }
}
