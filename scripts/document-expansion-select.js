...
const { selectDocumentExpansionPolicy } = require('./lib/document-expansion-selection');
...
function main(argv = process.argv.slice(2)) {
  ...
  const selection = selectDocumentExpansionPolicy({
    manifest,
    run1: readJson(options.run1Path, 'run 1'),
    run2: readJson(options.run2Path, 'run 2'),
    policies: readJson(options.policiesPath, 'policies'),
  });
  writeJsonAtomic(options.outputPath, selection);
  console.log(`[document-expansion-select] ${selection.status} ${options.outputPath}`);
  return selection;
}
...
function parseCliOptions(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    const [flag, inlineValue] = argv[index].split(/=(.*)/s, 2);
    const readValue = () => {
    ...
    switch (flag) {
      case '--manifest': values.manifestPath = readValue(); break;
      case '--run-1': values.run1Path = readValue(); break;
      case '--run-2': values.run2Path = readValue(); break;
      case '--policies': values.policiesPath = readValue(); break;
      case '--output': values.outputPath = readValue(); break;
      default: throw new Error(`unknown option: ${flag}`);
    }
  }
  for (const [property, flag] of [
    ['manifestPath', '--manifest'], ['run1Path', '--run-1'], ['run2Path', '--run-2'],
    ['policiesPath', '--policies'], ['outputPath', '--output'],
  ]) {
...
if (require.main === module) {
  try {
    ...
    console.error(`[document-expansion-select] ERROR ${error?.message ?? error}`);
    process.exitCode = 1;
  }
}

module.exports = { main, parseCliOptions, writeJsonAtomic };
