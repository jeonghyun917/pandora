...
function main(argv = process.argv.slice(2)) {
  ...
  console.log(`[group-balanced-bm25-select] ${selection.status} ${options.outputPath}`);
  return selection;
}
...
function parseCliOptions(argv) {
  ...
  for (const [property, flag] of [
    ['manifestPath', '--manifest'], ['run1Path', '--run-1'],
...
if (require.main === module) {
  try {
    ...
    console.error(`[group-balanced-bm25-select] ERROR ${error?.message ?? error}`);
    process.exitCode = 1;
  }
}
